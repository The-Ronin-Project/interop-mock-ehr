...
class R4DiagnosticReportResourceTest : BaseMySQLTest() {
    ...
    fun `date params without patient param returns empty list`() {
        val emptyRef = ReferenceParam("")
        val output = diagnosticReportProvider.search(
            patient = emptyRef,
            dateRangeParam = DateRangeParam()
        )
        assertEquals(0, output.size)

        val output2 = diagnosticReportProvider.search(
            patient = emptyRef,
    ...
    fun `search by with upper and lower bound date`() {
        val patientId = "1234"
        val diagnosticReport = DiagnosticReport() // should not find this one
        ...
        val diagnosticReport3 = DiagnosticReport()
