...
class STU3PatientAndBaseResourceTest : BaseMySQLTest() {

    private lateinit var collection: Collection
    private lateinit var patientProvider: STU3PatientResourceProvider
    private lateinit var dao: STU3PatientDAO
    ...
    fun initTest() {
        ...
        dao = STU3PatientDAO(database)
        patientProvider = STU3PatientResourceProvider(dao)
    }
    ...
    fun `update test with id`() {
        ...
        collection.add(FhirContext.forDstu3().newJsonParser().encodeResourceToString(testPat)).execute()
        ...
        val outputPat = FhirContext.forDstu3().newJsonParser().parseResource(Patient::class.java, dbDoc.toString())
    ...
    fun `return all test`() {
        ...
        collection.add(FhirContext.forDstu3().newJsonParser().encodeResourceToString(testPat1)).execute()

        val testPat2 = Patient()
        testPat2.birthDate = Date(87, 1, 15)
        testPat2.id = "TESTINGID11"
        collection.add(FhirContext.forDstu3().newJsonParser().encodeResourceToString(testPat2)).execute()
    ...
    fun `baseDAO findById test`() {
        ...
        collection.add(FhirContext.forDstu3().newJsonParser().encodeResourceToString(patient)).execute()
